...
The Noticia content type
===============================

In this section we are tesing the Noticia content type by performing
basic operations like adding, updadating and deleting Noticia content
items.

Adding a new Noticia content item
--------------------------------
...
Then we select the type of item we want to add. In this case we select
'Noticia' and click the 'Add' button to get to the add form.

    >>> browser.getControl('Noticia').click()
    >>> browser.getControl(name='form.button.Add').click()
    >>> 'Noticia' in browser.contents
    True

Now we fill the form and submit it.

    >>> browser.getControl(name='title').value = 'Noticia Sample'
    >>> browser.getControl('Save').click()
    >>> 'Changes saved' in browser.contents
    True

And we are done! We added a new 'Noticia' content item to the portal.

Updating an existing Noticia content item
---------------------------------------

Let's click on the 'edit' tab and update the object attribute values.

    >>> browser.getLink('Edit').click()
    >>> browser.getControl(name='title').value = 'New Noticia Sample'
    >>> browser.getControl('Save').click()
...
We check that the changes were applied.
    ...
    >>> 'Changes saved' in browser.contents
    True
    >>> 'New Noticia Sample' in browser.contents
    True

Removing a/an Noticia content item
--------------------------------

If we go to the home page, we can see a tab with the 'New Noticia
Sample' title in the global navigation tabs.
...
Now we are going to delete the 'New Noticia Sample' object. First we
go to the contents tab and select the 'New Noticia Sample' for
deletion.

    >>> browser.getLink('Contents').click()
    >>> browser.getControl('New Noticia Sample').click()
...
So, if we go back to the home page, there is no longer a 'New Noticia
Sample' tab.
...
Adding a new Noticia content item as contributor
------------------------------------------------

Not only site managers are allowed to add Noticia content items, but
also site contributors.
...
We select 'Noticia' and click the 'Add' button to get to the add form.
...
Done! We added a new Noticia content item logged in as contributor.
